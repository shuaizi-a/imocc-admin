    ...
    <el-card>
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column :label="$t('msg.excel.action')" fixed="right" width="260">
          <template #default="{ row }">
            <el-button type="primary" size="small" @click="onShowClick(row._id)">{{ $t('msg.excel.show', '查看') }}</el-button>
            <el-button type="info" size="small" @click="onShowRoleClick(row)" v-permission="['distributeRole']">{{
              $t('msg.excel.showRole', '角色')
            }}</el-button>
            <el-button type="danger" size="small" v-permission="['removeUser']" @click="onRemoveClick(row)">{{ $t('msg.excel.remove', '删除') }}</el-button>
          </template>
        </el-table-column>
      </el-table>
